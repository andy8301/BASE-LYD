...
export default function NexuraPage() {
  ...
  const canalesIngreso = ["NEXURA", "VENTANILLA", "CORREO CONTÁCTENOS", "VIRTUAL", "SADE"];
  ...
  const onSubmit = async (formData: any) => {
    try {
      const rowData = [
        ...
        formData.baseInformeNexura || "",           // B: BASE INFORME NEXURA
        ...
        formData.canalIngresoSecundario || "",      // I: Canal de ingreso
        ...
        formData.fechaRespuestaO || "",             // O: Fecha de respuesta (inicial)
        formData.fechaRespuestaA || "",             // P: Fecha de respuesta a
        "", "", "", "",                             // Q, R, S, T: Fórmulas (Días hábiles, transcurridos, total, Estado)
        ...
        "", "",                                     // AD, AE: Término, Requerimiento (Fórmulas/Automático)
        ...
        "",                                         // AJ: SI EL TIPO DE RENTA ES OTRO
        ...
        // AN en adelante: Prelación, semáforos, días pendientes y fórmulas automáticas de control
      ];

      if (editingItem) {
        const rowNumber = editingItem.id.replace('row-', '');
        await updateSheetRow("Base NEXURA", `A${rowNumber}:AM${rowNumber}`, rowData);
      } else {
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center bg-white p-4 rounded-lg shadow border">
        <div>
          ...
          <p className="text-sm text-gray-500">Gestión completa de radicación, solicitantes y trazabilidad gerencial</p>
        </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-5xl max-h-[92vh] overflow-y-auto">
          <DialogHeader className="border-b pb-4">
            <DialogTitle>Formulario de Registro Integral - Base NEXURA</DialogTitle>
          </DialogHeader>
          
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 pt-4 pb-10">
            
            {/* 1. Radicación e Identificación Básica */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-blue-50/50 rounded-lg border border-blue-100">
              <h3 className="col-span-3 font-bold text-blue-800 border-b border-blue-200 pb-1 text-sm">1. Radicación e Identificación</h3>
              ...
              <div className="space-y-1"><Label className="text-xs font-bold">Base Informe Nexura</Label><Input {...register("baseInformeNexura")} className="bg-white h-8" /></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-indigo-50/50 rounded-lg border border-indigo-100">
              <h3 className="col-span-3 font-bold text-indigo-800 border-b border-indigo-200 pb-1 text-sm">2. Datos del Solicitante</h3>
            ...
            {/* 3. Solicitud y Clasificación */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-orange-50/50 rounded-lg border border-orange-100">
              <h3 className="col-span-3 font-bold text-orange-800 border-b border-orange-200 pb-1 text-sm">3. Solicitud y Clasificación</h3>
            ...
            {/* 4. Gestión, Asignación y Respuesta */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-green-50/50 rounded-lg border border-green-100">
              <h3 className="col-span-3 font-bold text-green-800 border-b border-green-200 pb-1 text-sm">4. Gestión, Asignación y Respuesta</h3>
              ...
              <div className="space-y-1"><Label className="text-xs font-bold text-blue-700">Fecha de Respuesta</Label><Input {...register("fechaRespuestaAL")} type="date" className="bg-white h-8 border-blue-300" /></div>
              <div className="space-y-1"><Label className="text-xs font-bold">No. de SADE de Salida</Label><Input {...register("numeroSadeSalida")} className="bg-white h-8" /></div>
