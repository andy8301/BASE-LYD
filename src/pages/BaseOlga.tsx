  ...
  const onSubmit = async (formData: any) => {
    try {
      const rowData = [
        ...
        formData.fechaVencimientoBD || "", "", "", "", // BD, BE, BF, BG (Fórmulas)
    ...
    <div className="p-6 space-y-6">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="border-b pb-2"><DialogTitle>Formulario Integral (B a BJ)</DialogTitle></DialogHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pt-4 pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-blue-50/50 rounded-lg border border-blue-100">
              <h3 className="col-span-3 font-bold text-blue-800">1. Radicación e Ingreso</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-green-50/50 rounded-lg border border-green-100">
              <h3 className="col-span-3 font-bold text-green-800">2. Contribuyente y Renta</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-orange-50/50 rounded-lg border border-orange-100">
              <h3 className="col-span-3 font-bold text-orange-800">3. Trámite y Seguimiento</h3>
              <div className="space-y-1"><Label className="text-xs font-bold">Tipo Trámite (U)</Label><Input {...register("tipoTramite")} className="bg-white h-9" /></div>
              <div className="space-y-1"><Label className="text-xs font-bold">Prelación (X)</Label><Input {...register("prelacionLegal")} className="bg-white h-9" /></div>
              <div className="space-y-1"><Label className="text-xs font-bold">Respuesta (AC)</Label><Select onValueChange={(v) => setValue("tipoRespuesta", v)}><SelectTrigger className="bg-white h-9"><SelectValue placeholder="-" /></SelectTrigger><SelectContent>{respuestas.map(r => <SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent></Select></div>
              <div className="space-y-1"><Label className="text-xs font-bold">No. Resolución (Z)</Label><Input {...register("numeroResolucion")} className="bg-white h-9" /></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-yellow-50/50 rounded-lg border border-yellow-200">
              <h3 className="col-span-3 font-bold text-yellow-800">4. Control Final (BH-BJ)</h3>
              <div className="space-y-1"><Label className="text-xs font-bold">Semáforo (BH)</Label><Input disabled placeholder="Automático" className="bg-slate-100 h-9" /></div>
